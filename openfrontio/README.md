...
### Chrome extension (OpenFront Enhanced)

Optional QoL while playing or developing: [OpenFront Enhanced](https://github.com/marijnbent/openfront-extension) (spawn markers, keybinds, sounds, etc.). The workspace copy under `../openfront-extension` also includes an **OFE · Auto-join** panel (filter public lobbies and dispatch the same `join-lobby` event as the client; not affiliated with third-party store extensions). Load that directory as an unpacked extension in Chrome; content scripts run on **`http://localhost:9000`** as well. See the repository root [README](../README.md) for a short overview.
