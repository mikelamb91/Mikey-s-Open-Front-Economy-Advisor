// OpenFront context discovery, FPS saver patching, and selective trade policy.
  ...
  function findOpenFrontGameContextInDom() {
    const now = Date.now();
    if (now - lastDomDiscoveryAt < 8000) return null;
    lastDomDiscoveryAt = now;
    const gameSources = [];
  ...
  function getOpenFrontGameContext() {
    const now = Date.now();
    if (contextCache && now - contextCacheAt < 900) return contextCache;
    ...
    if (discoveredContext) {
      contextCache = discoveredContext;
      contextCacheAt = now;
    ...
    if (
      ...
      contextCache = lastOpenFrontGameContext;
      contextCacheAt = now;
      return lastOpenFrontGameContext;
    }
